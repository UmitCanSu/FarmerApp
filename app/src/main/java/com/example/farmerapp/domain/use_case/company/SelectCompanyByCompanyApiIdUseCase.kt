...
class SelectCompanyByCompanyApiIdUseCase
@Inject constructor(
    private val companyRepository: CompanyRepository
) {
    fun selectCompanyByCompanyApiId(companyId: String) = flow<Resource<Company?>> {
        emit(Resource.Loading())
        val company = companyRepository.selectCompanyByCompanyApiId(companyId)
        if (company == null){
